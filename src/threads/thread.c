...
void FP_ADD_MIX_OPERATION(struct thread *curr_t){
curr_t->recent_cpu = FP_ADD_MIX (curr_t->recent_cpu, 1);
}
void update_cpu_by_FP(struct thread *t){
...
void assert_idle_thread(struct thread *t){
  ASSERT (thread_mlfqs);
  ASSERT (t != idle_thread);
}

void thread_operation(struct thread *thread_){
  thread_->priority = FP_INT_PART (FP_SUB_MIX (FP_SUB (FP_CONST (PRI_MAX), FP_DIV_MIX (thread_->recent_cpu, 4)), 2 * thread_->nice));
  if (thread_->priority < PRI_MIN){
      thread_->priority = PRI_MIN;
  }else{
      thread_->priority = thread_->priority;
  }
 
  if (thread_->priority > PRI_MAX){
      thread_->priority = PRI_MAX;
  }else{
      thread_->priority = thread_->priority;
  } 
...
void re_update_recent_cpu(){
  ...
  for (; e != list_end (&all_list); e = list_next (e))
  {
    ...
    {
      update_cpu_by_FP(thread_);
if (thread_ == idle_thread){
    return;
}
  assert_idle_thread(thread_);

  thread_operation(thread_);
...
void thread_insert_order(struct thread *t){
...
bool compare_priority(const struct list_elem *thread1, const struct list_elem *thread2, void *aux UNUSED){
...
void check_block_status(struct thread *ele, void *temp UNUSED){

     if (ele->waiting_ticks > 0 && ele->status == THREAD_BLOCKED){
             ele->waiting_ticks--; 
